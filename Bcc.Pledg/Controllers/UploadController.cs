                ...
                using (OfficeOpenXml.ExcelPackage package = new OfficeOpenXml.ExcelPackage(stream))
                {
                    ...
                    for (int row = 1; row <= rowCount; row++)
                    {
                        var data = new PledgeReference
                        {
                            ...
                            ApartmentLayoutCode = worksheet.Cells[row, 9].Value != null ? worksheet.Cells[row, 9].Value.ToString() : null,
                            ApartmentLayout = worksheet.Cells[row, 10].Value != null ? worksheet.Cells[row, 10].Value.ToString() : null,
                            WallMaterialCode = Convert.ToInt32(worksheet.Cells[row, 11].Value),
                            WallMaterial = worksheet.Cells[row, 12].Value != null ? worksheet.Cells[row, 12].Value.ToString() : null,
                            DetailAreaCode = worksheet.Cells[row, 13].Value != null ? worksheet.Cells[row, 13].Value.ToString() : null,
                            DetailArea = worksheet.Cells[row, 14].Value != null ? worksheet.Cells[row, 14].Value.ToString() : null,
                            MinCostPerSQM = Convert.ToInt32(worksheet.Cells[row, 15].Value),
                            MaxCostPerSQM = Convert.ToInt32(worksheet.Cells[row, 16].Value),
                            Bargain = Convert.ToDecimal(worksheet.Cells[row, 17].Value),
                            MinCostWithBargain = Convert.ToInt32(worksheet.Cells[row, 18].Value),
                            MaxCostWithBargain = Convert.ToInt32(worksheet.Cells[row, 19].Value),
                            BeginDate = worksheet.Cells[row, 20].Value != null ? Convert.ToDateTime(worksheet.Cells[row, 20].Value) : (DateTime?)null,
                            EndDate = worksheet.Cells[row, 21].Value != null ? Convert.ToDateTime(worksheet.Cells[row, 21].Value) : (DateTime?)null
                        };
                        ...
                        var logData = new LogData
                        {
                            ...
                            Username = username,
                            PreviousId = _context.PledgeRefs.Any(p => p.Id == _context.PledgeRefs.Max(x => x.Id)) ?
